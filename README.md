...
---
# ✍ 느낀점
첫 프로젝트를 진행하며 데이터베이스 설계를 처음으로 직접 경험을 해보면서, 개발 과정에서 차질이 없도록 사전에 충분히 설계하는 것이 얼마나 중요한지 깨달았습니다. 소규모 프로젝트였음에도 불구하고, 테이블 구조와 데이터 흐름을 미리 명확하게 잡아두지 않으면 이후 단계에서 문제가 발생할 수 있다는 점을 체감했습니다. 특히, 이번 경험을 통해 프로젝트 설계가 개발의 성공을 좌우하는 핵심 단계라는 것을 확실히 이해하게 되었습니다.

또한, 연관관계 매핑 오류를 해결하는 과정에서 데이터베이스의 기본 개념과 관계 설정에 대해 깊이 이해하게 되었습니다. 오류를 해결하기 위해 다양한 접근 방식을 시도하며 학습한 점이 많았고, 이를 통해 데이터베이스 설계와 최적화에 대한 자신감을 얻었습니다. 문제를 해결하면서 성장할 수 있는 좋은 기회가 되었된 거 같습니다.

아쉬웠던 점은 Ajax를 프로젝트에 도입하지 못한 것입니다. 페이지를 리로드하지 않고 비동기적으로 데이터를 처리할 수 있었더라면 사용자 경험이 크게 향상되었을 텐데, 그 점이 많이 아쉽습니다. 앞으로는 Ajax를 적극 활용해 더 효율적인 사용자 인터페이스를 구현하는 것을 해보고 싶습니다.
